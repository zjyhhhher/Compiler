...
class SymTable{
   ...
   public:
      ...
      void exitScope(){
         if(table_.empty())fatalError("existScope:\
         Cannot remove scope from an empty table.");
         table_.pop_back();
      }
      void add(const K& k, const V& v){
         if(table_.empty())fatalError("add: Cannot \
         add a symbol without a scope.");
         table_.back()[k]=v;
      }
      std::optional<V> lookUp(const K& k){
      ...
      std::optional<V> probe(const K& k){
...
class Enviro{//整个classTable一个
   private:
      //？如果一个类没有method，那么会创造这个key出来吗？
      std::unordered_map<Symbol,std::unordered_map<Symbol,std::pair<Formals,Symbol>>>method_signatures_;
      SymTable<Symbol,Symbol> name_scope_;
      Symbol cur_class_;
   ...
   public:
      ...
      void setInheritGraph(std::unordered_map<Symbol,Symbol>& inherit_graph){
         inherit_graph_=inherit_graph;
      }
      void setCurClass(Symbol class_){
         cur_class_=class_;
      }
      ...
      void addFuncSig(Symbol class_name,Symbol method_name,Formals args,Symbol return_type){
      ...
      bool checkSubClass(Symbol class1,Symbol class2);
      Symbol getLca(const std::vector<Symbol>& classes);
};
