...
private:
   List<CgenNode> *nds;//记录所有的类
   ostream& str;
   int stringclasstag;
   int intclasstag;
   int boolclasstag;

   std::vector<Symbol> tag_to_class;//class_tag对应到class
   std::unordered_map<Symbol,std::pair<int,int>> subclass_idrange;//某个类的子类的tag range
   std::unordered_map<Symbol,std::unordered_map<Symbol,int>> class_attr_offset;//类中成员变量的次序（偏移）
   std::unordered_map<Symbol,std::unordered_map<Symbol,int>> class_method_offset;//类中成员函数的次序（偏移）
   int label_id{0};
...
public:
   ...
   std::pair<int,int> get_subclass_idrange(Symbol class_name){
...
public:
   Loc getSymbolToLoc(Symbol name){
